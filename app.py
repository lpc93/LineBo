...
from linebot.models import *
from google_weather import get_weather  # 引入google-weather庫

app = Flask(__name__)

# Line Bot API 和 Webhook Handler 設置
line_bot_api = LineBotApi(os.getenv('CHANNEL_ACCESS_TOKEN'))
handler = WebhookHandler(os.getenv('CHANNEL_SECRET'))
...
def get_google_weather(city_name):
    weather = get_weather(city_name, lang='zh-tw')  # 獲取繁體中文天氣資訊
    if weather:
...
def handle_message(event):
    ...
    if msg.startswith("天氣"):
        ...
        if city_name:
            weather_info = get_google_weather(city_name)
